// использует FORMS- аутентификацию на основе jwt-токена (заголовок, полезная нагрузка, подпись)
const fs = require("fs");
...
const jwtExpirySeconds = 60; // 10 минут
...
function signIn(req, res) {
    ...
    res.cookie("token", token, { maxAge: jwtExpirySeconds * 1000 }); // время жизни access-токена 
...
function welcome(req, res) {
	...
	var payload;
	try {
		payload = jwt.verify(token, jwtKey);    // проверка jwt на действительную подпись(возврат декодированные полезные данные)
	} catch (err) {
        return res.redirect("/login");
	}
